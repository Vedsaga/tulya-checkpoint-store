    ...
    for (line_no, line) in BufReader::new(File::open(path)?).lines().enumerate() {
        ...
        let get = |name: &str| {
            row.get(name)
                ...
                .ok_or_else(|| format!("history row missing {name}"))
        };
        let op = Op {
            version: get("version")?,
            parent: get("parent")?,
        ...
        if op.version != expected || op.parent >= op.version {
            return Err("history topology is not dense/prior-parent".into());
        }
        if op.length == 0
            || op
                .offset
                ...
                > base_len
        {
            return Err("edit range exceeds base".into());
        }
        ops.push(op);
    }
    Ok(ops)
...
fn committed(outcome: CommitOutcome) -> Result<Version, Box<dyn Error>> {
    outcome
        .version()
        .ok_or_else(|| "benchmark commit produced no version".into())
}
...
fn node_file_bytes(root: &Path) -> Result<u64, Box<dyn Error>> {
    ...
    for e in fs::read_dir(root)? {
        ...
        if n.starts_with("content-") && n.ends_with(".nodes") {
            total = total.checked_add(m.len()).ok_or("node bytes overflow")?;
        }
    }
    Ok(total)
}
fn node_records(bytes: u64) -> Result<u64, Box<dyn Error>> {
    if bytes < NODE_HEADER_BYTES || (bytes - NODE_HEADER_BYTES) % NODE_RECORD_BYTES != 0 {
        return Err("node file alignment invalid".into());
    }
...
fn storage_breakdown(root: &Path) -> Result<Value, Box<dyn Error>> {
    ...
    for e in fs::read_dir(root)? {
        ...
        let category = if name.starts_with("content-") && name.ends_with(".payload") {
            payload += bytes;
            "payload"
    ...
    Ok(
        json!({"total_bytes":total,"payload_file_bytes":payload,"node_file_bytes":nodes,"wal_file_bytes":wal,"snapshot_file_bytes":snapshot,"manifest_file_bytes":manifest,"other_file_bytes":other,"files":files}),
    )
}

fn read_edit(file: &mut File, version: u64, length: usize) -> Result<Vec<u8>, Box<dyn Error>> {
    let offset = (version - 1)
        .checked_mul(u64::try_from(length)?)
        .ok_or("edit corpus offset overflow")?;
...
fn sample_versions(updates: usize, samples: usize) -> Vec<u64> {
    let count = samples.max(1).min(updates);
    if count == 1 {
        return vec![u64::try_from(updates).unwrap_or(1)];
    }
    (0..count)
        .map(|i| u64::try_from(1 + i * (updates - 1) / (count - 1)).unwrap_or(1))
        .collect()
}
...
fn run(args: &Args) -> Result<Value, Box<dyn Error>> {
    ...
    let ops = parse_history(&args.history, base_len)?;
    if ops.is_empty() {
        return Err("empty history".into());
    }
    let edit_bytes = usize::try_from(ops[0].length)?;
    if ops
        .iter()
        .any(|o| usize::try_from(o.length).ok() != Some(edit_bytes))
    {
        return Err("benchmark requires fixed edit length".into());
    }
    let expected_edits = u64::try_from(ops.len())?
        .checked_mul(u64::try_from(edit_bytes)?)
        .ok_or("edit corpus size overflow")?;
    if fs::metadata(&args.edits)?.len() != expected_edits {
        return Err("edit corpus length mismatch".into());
    }
    ...
    let t = Instant::now();
    let root = committed(authority.splice(history_id, None, 0, 0, &base, None, None)?)?;
    let root_create_ns = t.elapsed().as_nanos();
    drop(base);
    let initial_node_file_bytes = node_file_bytes(&args.db)?;
    let initial_node_records = node_records(initial_node_file_bytes)?;
    let mut prev = initial_node_records;
    let mut versions = Vec::with_capacity(ops.len() + 1);
    versions.push(root);
    let mut update_times = Vec::with_capacity(ops.len());
    let mut allocations = Vec::with_capacity(ops.len());
    let mut edits = File::open(&args.edits)?;
    for op in &ops {
        let parent = *versions
        ...
        let t = Instant::now();
        let child = committed(authority.splice(
            history_id,
            Some(parent.id()),
            op.offset,
            op.length,
        ...
        update_times.push(t.elapsed().as_nanos());
        if child.id().id() != op.version {
            return Err("Tulya version id mismatch".into());
        }
        versions.push(child);
        let now = node_records(node_file_bytes(&args.db)?)?;
        allocations.push(now.checked_sub(prev).ok_or("node count regressed")?);
        prev = now;
    }
    let final_node_file_bytes = node_file_bytes(&args.db)?;
    let final_node_records = node_records(final_node_file_bytes)?;
    let fresh = final_node_records - initial_node_records;
    if allocations.iter().copied().sum::<u64>() != fresh {
        return Err("node allocation accounting mismatch".into());
    }
    let pre = storage_breakdown(&args.db)?;
    let pre_bytes = pre["total_bytes"].as_u64().ok_or("bad pre total")?;
    let t = Instant::now();
    let seal = authority.seal()?;
    let seal_ns = t.elapsed().as_nanos();
    let post = storage_breakdown(&args.db)?;
    let post_bytes = post["total_bytes"].as_u64().ok_or("bad post total")?;
    drop(authority);
    let t = Instant::now();
    ...
    let mut read4 = Vec::new();
    let mut read_edit_times = Vec::new();
    let mut edits = File::open(&args.edits)?;
    for version_id in sample_versions(ops.len(), args.verify_samples) {
        let op = ops[usize::try_from(version_id - 1)?];
        let expected = read_edit(&mut edits, version_id, edit_bytes)?;
        let version = reopened.store.lookup_version(VersionId::new(version_id))?;
        let t = Instant::now();
        let mut small = Vec::with_capacity(READ4K);
        reopened
            .store
            .read(version, op.offset, u64::try_from(READ4K)?, &mut small)?;
        read4.push(t.elapsed().as_nanos());
        let t = Instant::now();
        let mut whole = Vec::with_capacity(edit_bytes);
        reopened
            .store
            .read(version, op.offset, op.length, &mut whole)?;
        read_edit_times.push(t.elapsed().as_nanos());

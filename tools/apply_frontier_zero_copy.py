...
AVl = "crates/tulya-core/src/persistent_sequence/avl.rs"
PHYSICAL = "crates/tulya-core/src/persistent_sequence/physical.rs"
COMPACTION = "crates/tulya-core/src/persistent_sequence/compaction_v2.rs"

# 1. Give physical arenas a way to represent a fresh leaf as a view onto an
# existing immutable payload range. The pure in-memory arena keeps the old
# copy-on-split behavior by using this default implementation.
replace_once(
    AVl,
...
# 2. Durable physical arenas alias old immutable payload. Fresh inserted bytes
# still append normally.
needle = '''    fn arena_read_payload(&self, start: u64, end: u64) -> Result<Vec<u8>, V2AvlError> {
        self.read_payload_range(start, end)
    }

    fn arena_append_payload(&mut self, bytes: &[u8]) -> Result<u64, V2AvlError> {
'''
replacement = '''    fn arena_read_payload(&self, start: u64, end: u64) -> Result<Vec<u8>, V2AvlError> {
        self.read_payload_range(start, end)
    }
    ...
    fn arena_append_payload(&mut self, bytes: &[u8]) -> Result<u64, V2AvlError> {
'''
# This exact block belongs only to PhysicalContentStore; DeltaView has a
# different arena_read_payload body and is patched separately below.
replace_once(PHYSICAL, needle, replacement)
...
# 3. GC/compaction must treat overlapping immutable leaf views as legitimate,
# copy only their byte union, and retain an exact mapping for every leaf view.
replace_once(
    COMPACTION,
...
old_compact = '''/// Generic payload-repack core shared by checkpoint compaction and
...
'''
new_compact = '''/// Generic payload-repack core shared by checkpoint compaction and
...
pub(super) fn repack_compact_ranges(
    ...
    for range in ranges {
        ...
        total = total.checked_add(contribution).ok_or(
            V2CompactionError::Overflow("v2 compaction compact payload length exceeds u64"),
        )?;
...
'''
replace_once(COMPACTION, old_compact, new_compact)
...
print("zero-copy persistent-leaf source updates applied exactly")
